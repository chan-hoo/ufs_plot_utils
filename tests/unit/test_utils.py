...
def test_normalize_yaxis(sample_da_tile):
    ...
    assert da.dims == ("tile", "y", "x"), f'''Expected dims ("tile", "y", "x"), got {da.dims}'''
    assert da.shape == (6, 96, 96), f'''Expected shape (6, 96, 96), got {da.shape}'''
...
def test_normalize_grid(sample_da_grid):
    ...
    assert da.dims == ("tile", "y", "x"), f'''Expected normalized dims ("tile", "y", "x"), got {da.dims}'''
    assert da.shape == (6, 96, 96), f'''Expected shape (6, 96, 96), got {da.shape}'''
...
def test_normalize_preserves_values(sample_da_tile):
    ...
    assert np.allclose(sample_da_tile.values, da2.values), f'''Normalized data should preserve original values'''
    assert np.array_equal(sample_da_tile.values, da2.values), f'''Expected exact value preservation for all elements'''
...
def test_normalize_already_normalized():
    ...
    assert da_normalized.dims == ("tile", "y", "x"), f'''Already normalized dims should remain unchanged'''
...
def test_normalize_various_shapes(shape):
    ...
    da_normalized = normalize_tile_dims(da)
    assert da_normalized.shape == shape, f'''Shape {shape} should be preserved after normalization, got {da_normalized.shape}'''
    assert da_normalized.dims == ("tile", "y", "x"), f'''Dims should be normalized to ("tile", "y", "x") for shape {shape}'''
